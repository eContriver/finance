...
class Alpaca(Adapter):
    ...
    def translate_series(self, response_data, value_type: ValueType, key: str, data_date_format='%Y-%m-%d') -> None:
        ...
        # translated = {}
        # assert not response_data.df.empty, "The response data was empty"
        # to_translate = response_data.df.copy()
        # to_translate.index = pandas.to_datetime(to_translate.index.get_level_values('timestamp'))
        # # translated = translated.apply(self.)
        # to_translate = response_data.df.to_dict(orient='index')
