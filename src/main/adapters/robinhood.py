...
class Robinhood(Order):
    ...
    def __init__(self, symbol: str, base_symbol: str = 'USD',
        ...
        if environ.get('ROBINHOOD_PASSWORD') is None:
            raise RuntimeError(
                "The ROBINHOOD_PASSWORD environment variable is not set - this needs to be set to the API KEY for "
                "your account from the robinhood.co site.")
        password = environ.get('ROBINHOOD_PASSWORD')
        username = environ.get('ROBINHOOD_USERNAME')
    ...
    def get_is_digital_currency(self) -> bool:
        data, data_file = self.get_api_response(rs.robinhood.get_currency_pairs, {})
        is_digital = False
        for item in data:
    ...
    def place_sell_stop_order(self, quantity: float, price: float):
        ...
        # if self.asset_type == AssetType.EQUITY:
        #     raw_response, data_file = self.get_api_response(rs.robinhood.order_sell_stop_loss, args)
        # else:
        #     raise RuntimeError("Adapter is set with unsupported asset type:{}".format(self.asset_type))
        data = self.translate_order(raw_response)
        return data
    ...
    def get_historic_value(self) -> Dict[datetime, float]:
        ...
        raw_response, data_file = self.get_api_response(rs.robinhood.get_historical_portfolio, args, cache=False)
        assert raw_response, "The response data was empty"
    ...
    def get_stock_positions(self):
        ...
        raw_response, data_file = self.get_api_response(rs.robinhood.build_holdings, args, cache=False)
        assert raw_response, "The response data was empty"
        data = {}
    ...
    def get_crypto_positions(self):
        ...
        raw_response, data_file = self.get_api_response(rs.robinhood.get_crypto_positions, args, cache=False)
        assert raw_response, "The response data was empty"
        data = {}
    ...
    def get_open_stock_orders(self):
        ...
        raw_response, data_file = self.get_api_response(rs.robinhood.get_all_open_stock_orders, args, cache=False)
        orders = []
        for item in raw_response:
        ...
        return orders
        # orders = self.translate_orders(raw_response, rs.robinhood.get_stock_order_info)
        # return orders

    def get_open_crypto_orders(self):
        args = {
        }
        raw_response, data_file = self.get_api_response(rs.robinhood.get_all_open_crypto_orders, args, cache=False)
        orders = []
        for item in raw_response:
            side = OrderSide.BUY if item['side'] == 'buy' else OrderSide.SELL
            data_date_format = '%Y-%m-%dT%H:%M:%S.%f%z'
            ...
            pair_response, pair_file = self.get_api_response(rs.robinhood.get_currency_pairs, args)
            ...
            if item['type'] == 'limit':
                ...
                raise RuntimeError("Unsupported order type: {}".format(item['type']))
        return orders
        # orders = self.translate_orders(raw_response, rs.robinhood.get_crypto_order_info)
        # return orders
