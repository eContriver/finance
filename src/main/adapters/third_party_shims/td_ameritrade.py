...
#
#
# class TDAmeritrade(Series, Order):
#     span: str
#
#     def __init__(self, symbol: str, base_symbol: str = 'USD',
#                  cache_key_date: Optional[datetime] = None, span: Optional[timedelta] = None):
#         if environ.get('TDAMERITRADE_PASSWORD') is None:
#             raise RuntimeError(
#                 "The TDAMERITRADE_PASSWORD environment variable is not set - this needs to be set to the API KEY for "
#                 "your account from the robinhood.co site.")
#         password = environ.get('TDAMERITRADE_PASSWORD')
#         username = environ.get('TDAMERITRADE_USERNAME')
#         rs.tda.login(username, password)
...
#         data, data_file = self.get_api_response(rs.tda.get_stock_quote_by_symbol, {'symbol': self.symbol})
#         return 'trading_halted' in data and not data['trading_halted']
#
#     def get_is_digital_currency(self) -> bool:
#         data, data_file = self.get_api_response(rs.tda.get_currency_pairs, {})
#         is_digital = False
#         for item in data:
...
#             raw_response, data_file = self.get_api_response(rs.tda.order_buy_crypto_limit, args)
#         elif self.asset_type == AssetType.EQUITY:
#             raw_response, data_file = self.get_api_response(rs.tda.order_buy_limit, args)
#         else:
...
#         }
#         if self.asset_type == AssetType.DIGITAL_CURRENCY:
#             raw_response, data_file = self.get_api_response(rs.tda.order_sell_crypto_limit, args)
#         elif self.asset_type == AssetType.EQUITY:
#             raw_response, data_file = self.get_api_response(rs.tda.order_sell_limit, args)
#         else:
#             raise RuntimeError("Adapter is set with unsupported asset type:{}".format(self.asset_type))
#         data = self.translate_order(raw_response)
#         return data
...
#         }
#         if self.asset_type == AssetType.DIGITAL_CURRENCY:
#             raw_response, data_file = self.get_api_response(rs.tda.order_buy_crypto_by_quantity, args)
#         elif self.asset_type == AssetType.EQUITY:
#             raw_response, data_file = self.get_api_response(rs.tda.order_buy_market, args)
#         else:
#             raise RuntimeError("Adapter is set with unsupported asset type:{}".format(self.asset_type))
#         data = self.translate_order(raw_response)
#         return data
...
#         }
#         if self.asset_type == AssetType.DIGITAL_CURRENCY:
#             raw_response, data_file = self.get_api_response(rs.tda.order_sell_crypto_by_quantity, args)
#         elif self.asset_type == AssetType.EQUITY:
#             raw_response, data_file = self.get_api_response(rs.tda.order_sell_market, args)
#         else:
#             raise RuntimeError("Adapter is set with unsupported asset type:{}".format(self.asset_type))
#         data = self.translate_order(raw_response)
#         return data
...
#             raw_response, data_file = self.get_api_response(rs.tda.order_buy_stop_loss, args)
#         else:
#             raise RuntimeError("Adapter is set with unsupported asset type:{}".format(self.asset_type))
#         data = self.translate_order(raw_response)
#         return data
...
#             "stopPrice": price,
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.order_sell_crypto_by_price, args)
#         # if self.asset_type == AssetType.EQUITY:
#         #     raw_response, data_file = self.get_api_response(rs.tda.order_sell_stop_loss, args)
#         # else:
#         #     raise RuntimeError("Adapter is set with unsupported asset type:{}".format(self.asset_type))
#         data = self.translate_order(raw_response)
#         return data
...
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.get_stock_historicals, args)
...
#             "interval": self.translate_interval(),
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.get_crypto_historicals, args)
...
#             # 'bounds': 'extended',
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.get_historical_portfolio, args, cache=False)
#         assert raw_response, "The response data was empty"
...
#         args = {
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.load_phoenix_account, args, cache=False)
#         assert raw_response, "The response data was empty"
#         data = {
...
#         args = {
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.build_holdings, args, cache=False)
#         assert raw_response, "The response data was empty"
#         data = {}
...
#         args = {
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.get_crypto_positions, args, cache=False)
#         assert raw_response, "The response data was empty"
#         data = {}
...
#         args = {
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.get_all_open_stock_orders, args, cache=False)
#         orders = []
#         for item in raw_response:
...
#                 'url': item['instrument']
#             }
#             instrument_response, instrument_file = self.get_api_response(rs.tda.get_instrument_by_url, args)
#
#             if item['type'] == 'limit':
...
#         return orders
#         # orders = self.translate_orders(raw_response, rs.tda.get_stock_order_info)
#         # return orders
#
#     def get_open_crypto_orders(self):
#         args = {
#         }
#         raw_response, data_file = self.get_api_response(rs.tda.get_all_open_crypto_orders, args, cache=False)
#         orders = []
#         for item in raw_response:
#             side = OrderSide.BUY if item['side'] == 'buy' else OrderSide.SELL
#             data_date_format = '%Y-%m-%dT%H:%M:%S.%f%z'
...
#             args = {
#             }
#             pair_response, pair_file = self.get_api_response(rs.tda.get_currency_pairs, args)
#
#             symbol = None
...
#                 raise RuntimeError("Unsupported order type: {}".format(item['type']))
#         return orders
#         # orders = self.translate_orders(raw_response, rs.tda.get_crypto_order_info)
#         # return orders
#
...
#     #     if info_api == rs.tda.get_stock_order_info:
